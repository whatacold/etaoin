...
Ivan Grishaev (https://github.com/igrishaev[@igrishaev]) created Etaoin and published its first release to Clojars in February 2017.
...
[TIP]
====
If it is not already installed, you will need to install the web browser (Chrome, Firefox, Edge).
This is usually done via a download from the browser's official site.
...
The Internet is constantly changing, making testing against live sites impractical.
The code in this user guide has been tested to work against our link:{url-sample-page}[little sample page].
...
TIP: As mentioned, we recommend the `with-<browser>` convention when you need proper cleanup.
...
* `:fn/index`: Takes a positive integer parameter.
...
* `:fn/has-string`: Takes a string parameter.
  ...
  Generally, if you are targeting an element at the top of the hierarchy, you probably want `:fn/has-string`.
...
Everything in the `template` element is part of the shadow DOM.
...
A double-click can be simulated with the `double-click` function.
...
This is useful to imitate holding a system key like Control, Shift, or whatever when typing.
...
They accept default timeout/interval values that can be redefined using the `with-wait-timeout` and `with-wait-interval` macros.
...
One controls the keyboard, the other controls the pointer (mouse).
...
The `screenshot` function dumps the currently visible page to a PNG image file on your disk.
...
The `:state` field has the same semantics of the `XMLHttpRequest.readyState`.
...
The function `dev/get-performance-logs` returns a list of logs, so you accumulate them in an atom or whatever:
...
_Default:_ Etaoin selects a random unused port when launching a local WebDriver process.
When connecting to a remote WebDriver process, it varies by vendor:
...
** only logs to a log file which Etaoin automatically discovers and populates as <<opt-driver-log-file>> in the `driver` map
...
This can lead to 403 responses or some weird behavior on the site.
...
It is not unusual for Continuous Integration services to have no display.
This is especially true for Linux runners.
...
To avoid copying and pasting the options map, declare it at the top of the module.
...
Sometimes, a file downloads automatically when you click a link or visit a page.
In tests, you might need to ensure a file has been downloaded successfully.
...
Etaoin can play the files produced by https://www.selenium.dev/documentation/ide/[Selenium IDE].
...
Once installed and activated, it records your actions to a JSON file with the `.side` extension.
...
The `:port` would be the appropriate port for the running WebDriver process as exposed by docker.
...
Vendors don't always do a great job of completely following or implementing the https://www.w3.org/TR/webdriver2/[W3C WebDriver Spec].

The https://wpt.fyi/results/webdriver/tests/classic[web-plaforms-tests dashboard] is a great way to get a feel for vendor support of the spec.
...
----
Cause:: This was a bug in `chromedriver` that was fixed in chromedriver v2.28.
